"""
Module 11 to maintain student repository of Universities
"""
import os
import sqlite3
...
from utilities.utility import file_reading_gen
...
class Repository:
    ...
    def __init__(self, name, path, pretty_print):
        """
        Fucntion to initiate the University Repository
        :param name: string specifying the name of university
        :param path: string sepcifying the directory to fetch univeristy detail
        ...
        if path is None:
            db_path = None
            path = os.getcwd()
            path = os.path.join(path, "utilities")
    ...
    def _populate_student_grades(self, path):
        ...
        try:
            ...
            for student_cwid, course_name, grade, instructor_cwid in grades_reader:
                ...
                if grade is None or ord(grade[0].upper()) - ord('A') > 2 or grade == 'C-':
                    continue
    ...
    def instructor_table_db(self, db_path):
        """
        Function to retrieve instructor and course information from database
        :param db_path: System filepath to fetch the database
        :return:
        """
        if db_path is None:
            db_path = os.path.join(os.getcwd(), "utilities", "810_startup.db")

        try:
            db_connection = sqlite3.connect(db_path)
        except sqlite3.OperationalError as error:

            print(f"ERROR: Not able to open the database {db_path}")
        else:
            instructor_summary = []
            ...
            try:
                for row in db_connection.execute(query):
                    data = {"cwid": row[0], "name": row[1], "dept": row[2],
                            "course": row[3], "count": row[4]}
                    instructor_summary.append(data)
                db_connection.close()
            except sqlite3.OperationalError as error:
                print(f"ERROR: Not able to open the database {db_path}")
